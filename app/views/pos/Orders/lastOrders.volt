<form>
    <div class="product_left shadow">
        <div class="box-table">
            <table cellpadding="0" cellspacing="0" id="last_order">
               <thead>
                  <tr>
                    ...
                    {% if(_list_orders) %}
                        {% for item in _list_orders %}
                            <tr>             
                                ...
                                <td class="price">{{item['dateCreated']}}</td>
                                <td class="price">{{item['price']}}</td>
                                <td class="price">
                                    <input type="button" value="Re-order" class="form-control btn" onclick="reorder('{{item['id']}}')">
                                </td>
                          </tr>          
                        {% endfor %}
                    {% else %}
    ...
    <div class="menu_right gray_bg" id="menu_scroll">
        <div class="right-menu-col">
            <div class="col-md-12 search_head">
                ...
                <div class="col-md-6 margin-top-30">
                    <input type="reset" value="Clear" onclick="window.location.href='{{ baseURLPos }}/poscash/orders/current'" class="btn-block form-control btn"/>
                </div>
            </div>
        </div>
    </div>
