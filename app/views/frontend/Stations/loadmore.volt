    ...
    <div class="bms_item col-xs-12 pos_{{ item['color_bar']}}" id="bmsitem_{{index}}" style="padding:10px;height:auto;">
        <div class="cartbox_des">
            <div class="cartbox_img">
                {% if item['collapse'] is defined and item['collapse']== 1 and item['arr_image'] is not empty  %}
            ...
            <div class="cartbox_name">
                ...
                ?>
                {% if item['collapse'] is defined and item['collapse']== 1 and item['line'] is not empty  %}
                    {% for kk,vv in item['line'] %}
                        <h4 style="font-size:18px;font-weight: bold;{% if vv['completed'] is defined and vv['completed'] ==1 %}color:green;{%endif%}"><b>{{ vv['quantity'] }}</b>. {{ vv['products_name'] }} 
                        {% if banh_mi_size>0 %} 
                           - size {{banh_mi_size}}" 
                        {% endif %}</h4>
                    ...
                    {% endfor %}
                {% else%}  
                    <h3 style="font-size:18px;font-weight: bold;{% if item['completed'] is defined and item['completed'] ==1 %}color:green{%endif%}">{{ item['products_name'] }} {% if banh_mi_size>0 %} 
                           - size {{banh_mi_size}}" 
                        {% endif %}</h3>
                    {% if item['options'] is defined and item['options']|length >0 %}
                        ...
                        {% endfor %}
                    {% endif %}
                    <div class="cart_note" style="font-size:16px;">
        ...
        <div class="cartbox_order">
            ...
            <br />
            <button style="margin-top:20px;" data-order-id="{{item['order_id']}}" class="coll_exp newbt {% if item['collapse']== 1 %} bt_expand bartop {%endif%}">{% if item['collapse'] is defined and item['collapse']== 1 %}Expand{%else%}Collapse{%endif%}</button>
        </div>
        <div class="cartbox_action">
            {% if item['collapse'] is defined and item['collapse']== 1%}
                <button class="btn up mainbt main_btns completeall" data-order-id="{{item['order_id']}}" data-is-manager="0" style="margin-top: -10px;">Complete All</button>
            {%else%}
...
{% endfor %}
    ...
    <div class="box_order_late">
        <p style="text-transform:uppercase;">Order late</p>
