...
<div class="level_{{n}} level_box" style="display:{% if n==1 %}block{% else %}none{% endif %}">
    <div class="popup_tab">
        ...
        {% if n>1 %}
            <button class="nextlevel" onclick="optionLevel({{n-1}})" style="display:block;{% if n<sum %}margin-right: 20%;{% endif %}">&laquo;Previous step</button>
        {% endif %}

        {% if n<sum %}
            <button class="nextlevel" onclick="optionLevel({{n+1}})" style="display:block">Continue &raquo;</button>
        {% endif %}
    </div>
